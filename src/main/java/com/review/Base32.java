...
public class Base32 {
    ...
    public String toString(byte [] b) {
        ...
        for (int i = 0; i < (b.length + 4) / 5; i++) {
            ...
            t[0] = (byte) ((s[0] >> 3) & 0x1F);                             // upper 5 bits from first byte
            t[1] = (byte) (((s[0] & 0x07) << 2) | ((s[1] >> 6) & 0x03));    // lower 3 bits from 1st byte, upper 2 bits from 2nd.
            t[2] = (byte) ((s[1] >> 1) & 0x1F);                             // bits 5-1 from 2nd.
            t[3] = (byte) (((s[1] & 0x01) << 4) | ((s[2] >> 4) & 0x0F));    // lower 1 bit from 2nd, upper 4 from 3rd
            t[4] = (byte) (((s[2] & 0x0F) << 1) | ((s[3] >> 7) & 0x01));    // lower 4 from 3rd, upper 1 from 4th.
            t[5] = (byte) ((s[3] >> 2) & 0x1F);                             // bits 6-2 from 4th
            t[6] = (byte) (((s[3] & 0x03) << 3) | ((s[4] >> 5) & 0x07));    // lower 2 from 4th, upper 3 from 5th;
            t[7] = (byte) (s[4] & 0x1F);                                    // lower 5 from 5th;
    ...
    public byte[] fromString(String str) {
        ...
        for (int i = 0; i < in.length / 8; i++) {
            ...
            t[0] = (s[0] << 3) | s[1] >> 2;                             // all 5 bits of 1st, high 3 (of 5) of 2nd
            t[1] = ((s[1] & 0x03) << 6) | (s[2] << 1) | (s[3] >> 4);    // lower 2 of 2nd, all 5 of 3rd, high 1 of 4th
            t[2] = ((s[3] & 0x0F) << 4) | ((s[4] >> 1) & 0x0F);         // lower 4 of 4th, high 4 of 5th
            t[3] = (s[4] << 7) | (s[5] << 2) | (s[6] >> 3);             // lower 1 of 5th, all 5 of 6th, high 2 of 7th
            t[4] = ((s[6] & 0x07) << 5) | s[7];                         // lower 3 of 7th, all of 8th
